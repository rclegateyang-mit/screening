#!/usr/bin/env python3
"""GMM estimation of (gamma, c) with V profiled out via share matching."""
...
def main() -> None:
    ...
    @jax.jit
    def jvp_log_odds(theta_gc: jnp.ndarray, direction: jnp.ndarray) -> jnp.ndarray:
        """Directional derivative of log-odds map along `direction`."""
        return jax.jvp(log_odds_profile, (theta_gc,), (direction,))[1]
    ...
    def moment_gc(theta_gc: jnp.ndarray) -> jnp.ndarray:
        ...
        _, vjp_fun = jax.vjp(log_odds_profile, theta_gc)
        return vjp_fun(R)[0]

    moment_gc = jax.jit(moment_gc)

    def gmm_objective_theta(theta_gc: jnp.ndarray) -> jnp.ndarray:
        m = moment_gc(theta_gc)
        return jnp.dot(m, m)

    gmm_objective_theta = jax.jit(gmm_objective_theta)

    def gmm_objective_z(z_vars: jnp.ndarray) -> jnp.ndarray:
        theta_gc = fwd(z_vars)
        return gmm_objective_theta(theta_gc)

    gmm_objective_z = jax.jit(gmm_objective_z)

    solver = LBFGS(fun=gmm_objective_z, value_and_grad=False, maxiter=int(args.maxiter), tol=args.tol)
    ...
    obj = float(gmm_objective_z(z_hat))
    ...
    def moments_fn(th_vec: np.ndarray):
        ...
        theta_gc = th_vec
        theta_gc_jax = jnp.asarray(theta_gc, dtype=jnp.float64)
        m_vec = np.asarray(moment_gc(theta_gc_jax))

        theta_full, _ = theta_full_from_gc(theta_gc_jax)
        P = compute_choice_probabilities_jax(theta_full, X, aux)
        R = Y_jax[:, 1:] - P[:, 1:]
        R_const = jax.lax.stop_gradient(R)

        K = theta_gc_jax.size
        basis = jnp.eye(K, dtype=theta_gc_jax.dtype)
        Psi_cols = []
        for idx in range(K):
            direction = basis[idx]
            jac_slice = jvp_log_odds(theta_gc_jax, direction)
            Psi_cols.append(jnp.sum(jac_slice * R_const, axis=1))

        Psi = jnp.stack(Psi_cols, axis=1)
    ...
    out_path = os.path.join(args.out_dir, 'gmm_gamma_c_profile_estimates_jax.json')
