...
if __package__ is None or __package__ == "":
    ...
    from screening import get_data_subdir, get_output_subdir, DATA_RAW, DATA_CLEAN, DATA_BUILD, OUTPUT_WORKERS  # type: ignore
    ...
    from ..analysis.lib.helpers import compute_order_maps, read_params_long_csv
...
def construct_worker_dataset(
    ...
    # Log-additive skill distribution: ln q = gamma0 + gamma1*v + e
    ...
    sigma_e = float(params.get('sigma_e', 0.135))
    ...
    mu_lnq = gamma0 + gamma1 * mu_v
    sigma_lnq = np.sqrt(gamma1**2 * sigma_v**2 + sigma_e**2)

    # Set up random number generator
    rng = np.random.default_rng(seed)

    print(f"Drawing {N_workers} workers from continuous distributions...")
    print(f"  v ~ N({mu_v:.3f}, {sigma_v:.3f}²)  (observable component)")
    print(f"  e ~ N(0, {sigma_e:.3f}²)  (unobservable component)")
    print(f"  ln q = {gamma0:.3f} + {gamma1:.3f}*v + e  =>  ln q ~ N({mu_lnq:.3f}, {sigma_lnq:.3f}²)")
    ...
    # Draw observable skill component v (possibly correlated with location)
    if abs(rho_x_skill_r) > 0.0:
        ...
        joint_mean = np.array([mu_v, worker_mu_x, worker_mu_y])
        joint_cov = np.array(
            [
                [sigma_v**2,
        ...
        try:
            ...
            raise ValueError(
                "Invalid correlation structure for (v, ell_x, ell_y); "
        ...
        v = rng.normal(mu_v, sigma_v, N_workers)
        ell_x, ell_y = _draw_locations()

    # 3) Draw unobservable component e ~ N(0, sigma_e²)
    e_skill = rng.normal(0.0, sigma_e, N_workers)

    # 4) Compute log skill and skill in levels
    ln_q = gamma0 + gamma1 * v + e_skill
    q = np.exp(ln_q)

    # Create DataFrame with basic worker characteristics
    # x_skill column stores v (observable component) for backward compatibility
    df = pd.DataFrame({
    ...
    if firms_data is not None:
        ...
        for i in range(N_workers):
            ...
            # Find feasible firms where q_i > qbar_j (equivalently ln_q_i > ln_qbar_j)
            feasible_firms = []
            feasible_utilities = []
    ...
    print(f"μ_s: {mu_lnq:.3f}, σ_s: {sigma_lnq:.3f}")
    print(f"σ_v: {sigma_v:.3f}, σ_e: {sigma_e:.3f}")
    print(f"γ₀: {gamma0:.3f}, γ₁: {gamma1:.3f}")
    print(
    ...
    # Basic statistics
    print(f"v (x_skill): mean={v.mean():.3f}, std={v.std():.3f}")
    print(f"ell_x: mean={ell_x.mean():.3f}, std={ell_x.std():.3f}")
    print(f"ell_y: mean={ell_y.mean():.3f}, std={ell_y.std():.3f}")
    print(f"e (a_skill): mean={e_skill.mean():.3f}, std={e_skill.std():.3f}")
    print(f"q (s_skill): mean={q.mean():.3f}, std={q.std():.3f}")

    # Compute empirical correlation for validation
    empirical_corr = np.corrcoef(ell_x, ell_y)[0, 1]
    corr_x_skill_ell_x = np.corrcoef(v, ell_x)[0, 1]
    corr_x_skill_ell_y = np.corrcoef(v, ell_y)[0, 1]
    r_vals = np.sqrt((ell_x - worker_mu_x) ** 2 + (ell_y - worker_mu_y) ** 2)
    if r_vals.std() > 0:
        corr_x_skill_r = np.corrcoef(v, r_vals)[0, 1]
    else:
    ...
    df.attrs['summary'] = {
        ...
        'a_skill_mean': e_skill.mean(),
        'a_skill_std': e_skill.std(),
        's_skill_mean': q.mean(),
        's_skill_std': q.std()
    }
...
def main() -> int:
    ...
    summary_data = {
        'parameter': [
            'N_workers', 'mu_v', 'sigma_v', 'gamma0', 'gamma1', 'sigma_e',
        ...
        'value': [
            summary['N_workers'], summary['mu_v'], summary['sigma_v'],
            summary['gamma0'], summary['gamma1'], summary['sigma_e'], summary['mu_s'], summary['sigma_s'],
