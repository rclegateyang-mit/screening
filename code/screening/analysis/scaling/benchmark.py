...
Usage:
    # Single run
    python -m screening.analysis.scaling.benchmark --N 5000 --J 10

    # Grid sweep
    python -m screening.analysis.scaling.benchmark \
...
def generate_data(N: int, J: int, env: Dict[str, str], seed: int = 12345) -> Tuple[bool, float, float, str]:
    ...
    rc, t, rss, _, stderr = _run_module(
        "screening.simulate.01_prep_data",
        [
            "--N_workers", str(N),
            "--J", str(J),
            "--quad_n_x", "25",
    ...
    rc, t, rss, _, stderr = _run_module(
        "screening.clean.02_solve_equilibrium",
    ...
    # Step 3: draw workers
    rc, t, rss, _, stderr = _run_module(
        "screening.build.03_draw_workers",
        [],
        env,
...
def run_mle(env: Dict[str, str], maxiter: int, extra_args: List[str]) -> Tuple[bool, float, float, Optional[dict], str]:
    ...
    rc, wall, rss, stdout, stderr = _run_module(
        "screening.analysis.mle.run_pooled",
        args,
        env,
        label="MLE",
    )
...
def main() -> None:
    ...
    if args.out_csv:
        ...
        from screening import get_output_subdir, OUTPUT_ESTIMATION
