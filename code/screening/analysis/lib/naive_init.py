...
Six-step procedure that derives all starting values from data alone:
  0a. Wage regression -> gamma0, sigma_e, tilde_gamma
  0b. Firm average skill -> Q_j
  0c. Production function 2SLS -> alpha
  0d. Screening thresholds -> tilde_q_j
  0e. Conditional logit -> tau, delta_j
  0f. Wage equation 2SLS -> eta
...
def compute_pooled_naive_init(
    ...
    """Pooled 6-step naive initialization.
    ...
    # ==================================================================
    # Step 0d: Screening thresholds (per-market)
    #   Data-driven: tq_j ≈ tilde_gamma * v_low_j, where v_low_j is a
    #   low quantile of skills among workers matched to firm j.  The
    #   screening cutoff determines the minimum skill to qualify, so the
    #   lowest-skilled matched workers bound the cutoff from above.
    #   This places tq in the region where screening is active and the
    #   NLL gradient is nonzero — the FOC-based approach (using estimated
    #   gamma0/sigma_e) can land in a flat region where gradients vanish.
    # ==================================================================
    tilde_qs = []
    for md in market_datas:
        choice_np = np.asarray(md.choice_idx, dtype=np.int32)
        ...
        for j in range(md.J):
            ...
            if np.any(mask_j):
                ...
                # No matched workers: use market median
                tq[j] = np.nan
    ...
    # ==================================================================
    # Step 0e: Conditional logit (per-market)
    # ==================================================================
    deltas = []
    tau_sum = 0.0
    total_N = 0
    ...
    # ==================================================================
    # Step 0f: Wage equation 2SLS (pooled firms)
