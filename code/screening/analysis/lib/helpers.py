...
def inclusive_values_at_loc(ell: np.ndarray, w_sorted: np.ndarray, xi_sorted: np.ndarray,
                           loc_sorted: np.ndarray, alpha: float, tau: float) -> np.ndarray:
    """
    Compute inclusive values at location ell in by-c order.
    ...
    Args:
        ...
        tau: Distance decay parameter (commuting cost)
    ...
    # v_j = exp(-tau * dist + alpha * log(w_sorted) + xi_sorted)
    v[1:] = np.exp(-tau * distances + alpha * log_w + xi_sorted)
...
def conditional_LM_at_loc(v: np.ndarray, p: np.ndarray, m: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # Q[k] = sum_{i=k}^{J} q[i], R[k] = sum_{i=k}^{J} r[i]
...
def compute_choice_probabilities_at_loc(v: np.ndarray, p_x: np.ndarray, denom: np.ndarray) -> np.ndarray:
    """
    Compute choice probabilities P(x,ell) using suffix sums.
...
def compute_p_x_probabilities(x_skill: float, qbar_sorted: np.ndarray, gamma1: float, gamma0: float, sigma_e: float) -> np.ndarray:
    """
    Compute skill-based probabilities p_x using truncated normal CDF differences.

    Args:
        x_skill: Worker observable skill component (v)
        qbar_sorted: Sorted screening thresholds in log space (ln_qbar, J)
        gamma1: Skill loading parameter on observable
        gamma0: Skill intercept
        sigma_e: Standard deviation of unobservable component
    ...
    qbar_pad = np.empty(J + 2, dtype=np.float64)
    qbar_pad[0] = -np.inf
    qbar_pad[1:-1] = qbar_sorted
    qbar_pad[-1] = np.inf

    # Compute mu_lnq_i = gamma0 + gamma1*v
    a_x = gamma0 + gamma1 * x_skill

    # Compute standardized thresholds
    z = (qbar_pad - a_x) / sigma_e
...
def _numeric_jacobian_afp(theta, mvec_fn, *, rel_step=1e-6, abs_step=1e-5, bounds=None):
    """
    Compute D = d m(theta) / d theta' at 'theta' using scipy.optimize.approx_fprime.
    ...
    # Bounds handling (theta + eps <= ub - delta); supports (lb, ub) arrays or list[(lb_k, ub_k)]
    ...
    # approx_fprime expects scalar output; loop over coordinates of m(theta)
    ...
    for k in range(K):
        ...
        D[k, :] = approx_fprime(theta, f_k, epsilon=eps)  # row k is dm_k/dtheta'
    return D
...
def compute_gmm_standard_errors(
    ...
    bounds=None,                        # bounds for theta (e.g., tau in [0,1], others free)
    h_abs: float = 1e-5,
    h_rel: float = 1e-6,
    ridge: float = 1e-12,
) -> dict:
    """
    Compute GMM standard errors at theta_hat.
    ...
    moments_fn : callable
        Returns (m_vec, Psi) at theta for the estimator's moment system.
        m_vec in R^K is the sample mean of moments; Psi in R^{N x K} are per-observation moment rows.
    weighting : (K x K) array_like or None
    ...
    mode : {"robust","efficient"}
        ...
        "efficient": efficient GMM using Chamberlain instruments evaluated at theta_hat (theta_0=theta_hat).
        Efficient SEs use the Fisher-information formula V approx (D' S_g^{-1} D)^{-1} / N with s_ij(theta_hat) fixed.
        This corresponds to efficient GMM with Chamberlain optimal instruments.
    chamberlain_builder : callable
        Required if mode=="efficient". Returns instruments G(theta) with shape (N,J,K):
        s_ij,k(theta) = d/dtheta_k log(P_ij(theta)/P_i0(theta)).
    prob_eval : callable
        Required if mode=="efficient". Returns P_full(theta) with shape (N,J+1), outside at col 0.
    Y_full : ndarray
    ...
    -------
    dict with:
      "vcov": (K,K) variance-covariance of theta_hat
    ...
    # Coerce theta_hat and call moments function
    ...
    if mode == "robust":
        ...
        # Build instruments at theta_hat
        ...
        # Compute probabilities and residuals at theta_hat
        ...
        # Numerical Jacobian of m(theta) with instruments **fixed at theta_hat**
        G_fixed = G_hat
        def mvec_fn_eff(th):
            Pf = prob_eval(th)
...
def gh_nodes_weights_std_normal(n: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # Transform to standard normal: z = sqrt(2) * x, lambda_weights = w / sqrt(pi)
    z = np.sqrt(2) * x
...
def map_to_normal(mu: float, sigma: float, z: np.ndarray, lambda_weights: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """
    Map standard normal nodes and weights to N(mu, sigma^2) distribution.
    ...
    Returns:
        Tuple of (nodes, weights) for N(mu, sigma^2) distribution
    """
    # Transform nodes: x = mu + sigma * z
    x_nodes = mu + sigma * z
...
def load_weight_matrix(path: str | None, J: int) -> np.ndarray:
    """Load a JxJ weight matrix from csv/json/npy; default identity."""
    if path is None:
...
# =============================================================================
# GMM ESTIMATION FUNCTIONS (moved from estimate_gmm_tau_delta.py)
# =============================================================================
...
def read_firms_data(path: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """From firms CSV, read arrays (firm_ids, w, Y, A or logA, xi, loc_firms, qbar_data)."""
    df = pd.read_csv(path)
    ...
    # Screening thresholds qbar
    if 'qbar' in df.columns:
        qbar_data = df['qbar'].values
    elif 'c' in df.columns:
        qbar_data = df['c'].values
    else:
        raise ValueError("Neither 'qbar' nor 'c' column found in firms data")

    return firm_ids, w, Y, A, xi, loc_firms, qbar_data
...
def compute_worker_firm_distances(ell_x: np.ndarray, ell_y: np.ndarray,
    ...
    """Precompute worker->firm distances D_nat (NxJ) using worker (ell_x, ell_y) and firm locations (x_j, y_j)."""
    from scipy.spatial.distance import cdist

    worker_locs = np.column_stack([ell_x, ell_y])  # (N, 2)
    return cdist(worker_locs, loc_firms, metric='euclidean')  # (N, J)


def compute_cutoffs(w: np.ndarray, Y: np.ndarray, A: np.ndarray, alpha: float) -> np.ndarray:
    """
    Compute cutoffs from firms (natural order):
    log qbar_j = log w_j - log A_j - log(1-alpha) + (alpha/(1-alpha))*(log Y_j - log A_j)
    qbar_j = exp(log qbar_j)
    """
    ...
    log_qbar = (np.log(w_safe) - np.log(A_safe) - np.log(1 - alpha) +
             (alpha / (1 - alpha)) * (np.log(Y_safe) - np.log(A_safe)))
...
def compute_choice_probabilities(tau: float, delta: np.ndarray, distances: np.ndarray,
                               w: np.ndarray, Y: np.ndarray, A: np.ndarray, qbar: np.ndarray,
                               x_skill: np.ndarray, firm_ids: np.ndarray,
                               alpha: float, gamma1: float, gamma0: float, sigma_e: float,
                               delta_nat: np.ndarray | None = None) -> np.ndarray:
    """
    Probability kernel per worker i for a given tau.

    Args:
        tau: Distance decay parameter (commuting cost)
        delta: Firm attractiveness values (J,) in by-qbar order (for backward compatibility)
        distances: Worker-firm distances (N, J)
        w: Firm wages (J,)
        Y: Firm output (J,)
        A: Firm TFP (J,)
        qbar: Firm screening thresholds in log space (ln_qbar, J)
        x_skill: Worker observable skills (v, N)
        firm_ids: Firm IDs (J,)
        alpha: DRS curvature parameter
        gamma1: Skill loading parameter on observable
        gamma0: Skill intercept
        sigma_e: Std dev of unobservable component
        delta_nat: Optional firm attractiveness values (J,) in natural order

    Returns:
        P (N x J+1), rows workers, cols natural firm IDs.
    """
    N, J = distances.shape

    # 1) Compute cutoffs from firms (natural order)
    qbar_nat = compute_cutoffs(w, Y, A, alpha)

    # 2) Order by qbar (ascending)
    order_maps = compute_order_maps(qbar_nat)
    order_idx = order_maps['order_idx']
    inv_order = order_maps['inv_order']
    qbar_sorted = order_maps['c_sorted']

    # 3) Reindex arrays to by-qbar order
    if delta_nat is not None:
        # Use provided delta_nat (natural order) and reindex to by-qbar order
        if len(delta_nat) != J:
            raise ValueError(f"delta_nat length {len(delta_nat)} does not match number of firms {J}")
        delta_sorted = delta_nat[order_idx]
    else:
        # Fall back to previous default: use delta (by-qbar order)
        delta_sorted = delta[order_idx]

    distances_sorted = distances[:, order_idx]  # (N, J) in by-qbar order

    # 4) Compute v_ij for each worker i - VECTORIZED
    # v_0 = 1 (outside), v_{ij} = exp(-tau*d_{ij} + delta_{(j)}) for j>=1
    v_out = np.ones((N, 1))  # (N, 1)
    v_in = np.exp(-tau * distances_sorted + delta_sorted[None, :])  # (N, J)
    v_all = np.concatenate((v_out, v_in), axis=1)  # (N, J+1)

    # 5) Compute denominators denom_k(i) = sum_{m=0}^k v_{im}
    denom = np.cumsum(v_all, axis=1)  # (N, J+1)

    # 6) Build p_x(i) for each worker - FULLY VECTORIZED
    # Sentinels: qbar_0 = -inf, qbar_{J+1} = +inf
    qbar_pad = np.concatenate([[-np.inf], qbar_sorted, [np.inf]])  # (J+2,)

    # Compute mean log skill for all workers: mu_lnq_i = gamma0 + gamma1*v_i
    s = gamma0 + gamma1 * x_skill  # (N,)

    # Broadcast the z-scores in one shot
    z_hi = (qbar_pad[1:][None, :] - s[:, None]) / sigma_e  # (N, J+1)
    z_lo = (qbar_pad[:-1][None, :] - s[:, None]) / sigma_e  # (N, J+1)
...
def create_probability_evaluator(delta: np.ndarray, distances: np.ndarray, w: np.ndarray,
                               Y: np.ndarray, A: np.ndarray, qbar: np.ndarray, x_skill: np.ndarray,
                               firm_ids: np.ndarray, alpha: float, gamma1: float, gamma0: float, sigma_e: float) -> Callable:
    """
    Return an evaluator that accepts either scalar tau or parameter vector theta.

    Returns:
        Callable that accepts:
        - (i) scalar tau (backward compatibility), or
        - (ii) tuple (tau, delta_nat) or 1D array theta with theta[0]=tau, theta[1:]=delta_nat

    The evaluator returns P (N x J+1), rows workers, cols natural firm IDs.
    """
    J = len(firm_ids)
    ...
    def probs_worker_level(theta) -> np.ndarray:
        ...
        if isinstance(theta, (int, float)):
            # Backward compatibility: scalar tau
            tau = float(theta)
            delta_nat = None
            ...
            tau, delta_nat = theta
            tau = float(tau)
            delta_nat = np.asarray(delta_nat)
        elif isinstance(theta, np.ndarray) and theta.ndim == 1:
            # 1D array theta with theta[0]=tau, theta[1:]=delta_nat
            if len(theta) == 1:
                # Single element array - treat as scalar tau
                tau = float(theta[0])
                delta_nat = None
            elif len(theta) == J + 1:
                # Full parameter vector theta[0]=tau, theta[1:]=delta_nat
                tau = float(theta[0])
                delta_nat = theta[1:]
            else:
                raise ValueError(f"theta length {len(theta)} must be 1 (scalar tau) or J+1={J+1} (tau + delta_nat for {J} firms)")
        else:
            raise ValueError(f"Invalid theta type: {type(theta)}. Expected scalar, tuple (tau, delta_nat), or 1D array theta[0]=tau, theta[1:]=delta_nat")

        # Validate delta_nat shape if provided
        if delta_nat is not None:
            if len(delta_nat) != J:
                raise ValueError(f"delta_nat length {len(delta_nat)} does not match number of firms {J}")

        return compute_choice_probabilities(tau, delta, distances, w, Y, A, qbar, x_skill,
                                          firm_ids, alpha, gamma1, gamma0, sigma_e, delta_nat)
...
def build_choice_matrix(chosen_firm: np.ndarray, firm_ids: np.ndarray) -> np.ndarray:
    """
    Build Y (N x J+1) one-hot: Y[i, j_nat] = 1{chosen_firm_i == j_nat}.
...
def chamberlain_instruments_numeric(
    ...
    *,
    ...
    Compute Chamberlain optimal instruments via finite-difference Jacobian of
        F(theta) := vec( log P_{ij}(theta) - log P_{i0}(theta) )_{i=1..N, j=1..J}
    using scipy.optimize.approx_derivative.

    Parameters
    ----------
    theta : array_like (K,)
        Parameter vector (length K). Agnostic to contents (tau, delta, ...).
    prob_evaluator : callable
        Returns P_full(theta) with shape (N, J+1): column 0 = outside, columns 1..J = firms.
    ...
    G : ndarray, shape (N, J, K)
        Instruments array with entries d/dtheta_k log(P_{ij}/P_{i0}) at theta.

    Example
    -------
    Build prob_evaluator(theta) returning P_full (Nx(J+1)).
    Call G = chamberlain_instruments_numeric(theta, prob_evaluator, bounds=(lb,ub)).
    """
    ...
    assert np.allclose(row_sums, 1.0, atol=1e-10), f"Row sums must be approx 1, got range [{np.min(row_sums):.2e}, {np.max(row_sums):.2e}]"

    # Define vector-valued function F(theta) returning stacked log-odds against outside
    def F(theta_vec):
        """F(theta) = vec( log P[:,1:] - log P[:,[0]] ) with shape (N*J,)"""
...
# =============================================================================
# NAIVE INITIAL THETA (tau, delta, A) VIA MNL + SAMPLE AVERAGES
# =============================================================================

def _estimate_tau_delta_mnl(
    x_skill: np.ndarray,
    ell_x: np.ndarray,
    ell_y: np.ndarray,
    chosen_firm: np.ndarray,
    ...
    *,
    ...
    """Estimate (tau, delta) via MNL using worker choices and distances."""
    ...
    theta0 = np.concatenate(([float(tau0)], delta0))
    ...
    def nll_and_grad(theta_vec: np.ndarray) -> tuple[float, np.ndarray]:
        ...
        tau = float(theta_vec[0])
        delta_vals = theta_vec[1:]

        U = delta_vals[None, :] - tau * D
        ...
        nll = sum_log_denom - (sum_delta - tau * sum_d) + 0.5 * reg_delta * float(delta_vals @ delta_vals)
    ...
    bounds = [(tau_bounds[0], tau_bounds[1])] + [(-np.inf, np.inf)] * J
    res = minimize(
    ...
    tau_hat = float(theta_mnl[0])
    delta_hat = theta_mnl[1:]
    return tau_hat, delta_hat


def naive_theta_guess_tau_delta_A(
    x_skill: np.ndarray,
    ell_x: np.ndarray,
    ell_y: np.ndarray,
    chosen_firm: np.ndarray,
    ...
    *,
    ...
    maxiter: int = 500,
    verbose: bool = False,
    firms_csv_path: str = "data/clean/equilibrium_firms.csv",
    params_csv_path: str = "data/raw/parameters_effective.csv",
) -> np.ndarray:
    """
    Build a naive initial guess for theta = (tau, delta_1..delta_J, A_1..A_J) from worker data.

    - A_j: exp( log Y_j - (1-alpha)*log( xbar_j * L_j ) ), where
        xbar_j is average x_skill among workers who chose firm j (fallback to global mean),
        and (Y_j, L_j) are read from firms CSV (aligned by firm_id).
    - (tau, delta_j): maximum-likelihood estimate from a standard multinomial logit where
      U_{ij} = delta_j - tau*d_{ij}, outside option utility normalized to 0.
    ...
    Args:
      ...
      chosen_firm: (N,) choices with 0=outside and j in firm_ids for firm j.
      loc_firms: (J,2) firm locations aligned with firm_ids ascending.
      firm_ids: (J,) array of economic firm identifiers (1..J).
      alpha: DRS curvature parameter used in the A_j formula.
      tau0: initial value for tau.
      tau_bounds: bounds for tau in MNL estimation.
      reg_delta: L2 ridge on delta to stabilize rare choices (penalty 0.5*reg*||delta||^2).
    ...
    Returns:
      theta0: (1+2J,) array [tau, delta_1..delta_J, A_1..A_J] in natural firm order.
    """
    x_skill = np.asarray(x_skill, float).ravel()
    ell_x = np.asarray(ell_x, float).ravel()
    ell_y = np.asarray(ell_y, float).ravel()
    ...
    tau_hat, delta_hat = _estimate_tau_delta_mnl(
        x_skill=x_skill,
        ell_x=ell_x,
        ell_y=ell_y,
    ...
    # A_j via: log A_j = log Y_j - (1-alpha)*log( xbar_j * L_j )
    #          xbar_j = mean x_skill among workers choosing firm j (fallback: global mean)
    ...
    logA_guess = logY - (1.0 - float(alpha)) * log_xbarL
    A_guess = np.exp(logA_guess)

    # Stack final theta: [tau, delta, A]
    theta_full = np.concatenate(([tau_hat], delta_hat, A_guess))
    return theta_full


def naive_theta_guess_tau_delta_qbar(
    x_skill: np.ndarray,
    ell_x: np.ndarray,
    ell_y: np.ndarray,
    chosen_firm: np.ndarray,
    ...
    *,
    ...
    """Naive theta_0 = (tau, delta, ln_qbar) using choices, locations, and observables.

    If *gamma0*, *gamma1*, and *alpha* are provided, ln_qbar is computed from the
    screening FOC using only observed data (Y, w, worker matches):

        H_j  = sum_{i matched to j} exp(gamma0 + gamma1 * v_i)
        ln A_j = ln Y_j - (1 - alpha) * ln H_j
        ln qbar_j = ln w_j - ln A_j - ln(1-alpha) + alpha * ln H_j

    Otherwise falls back to reading qbar from the firms CSV (converted to ln).
    """
    ...
    if gamma0 is not None and gamma1 is not None and alpha is not None:
        # --- Compute ln_qbar from screening FOC using observables only ---
        # H_j = sum_{i matched to j} exp(gamma0 + gamma1 * v_i)
        H = np.zeros(J, dtype=float)
        for idx, fid in enumerate(firm_ids):
            mask = chosen_firm == int(fid)
            if np.any(mask):
                H[idx] = np.sum(np.exp(gamma0 + gamma1 * x_skill[mask]))

        # Read w, Y from firms CSV
        w = np.empty(J, dtype=float)
        ...
        # For firms with no matched workers, fall back to median of others
        valid = H > 0
        if not np.all(valid):
            H_fallback = np.median(H[valid]) if np.any(valid) else 1.0
            H[~valid] = H_fallback

        # ln A_j = ln Y_j - (1-alpha) * ln H_j
        ln_H = np.log(np.maximum(H, 1e-300))
        ln_A = np.log(np.maximum(Y, 1e-300)) - (1.0 - alpha) * ln_H

        # ln qbar_j = ln w_j - ln A_j - ln(1-alpha) + alpha * ln H_j
        ln_qbar_guess = (np.log(np.maximum(w, 1e-300))
                         - ln_A
                         - np.log(1.0 - alpha)
                         + alpha * ln_H)

        if verbose:
            print(f"[naive_qbar] H range: [{H.min():.2f}, {H.max():.2f}]")
            print(f"[naive_qbar] ln_qbar range: [{ln_qbar_guess.min():.4f}, {ln_qbar_guess.max():.4f}]")
    else:
        # --- Fall back to reading qbar from firms CSV (convert to log) ---
        qbar_col = 'qbar' if 'qbar' in df_firms.columns else 'c'
        if 'firm_id' not in df_firms.columns or qbar_col not in df_firms.columns:
            raise ValueError("Firms CSV must contain 'firm_id' and 'qbar' (or 'c') columns to build cutoff guesses.")

        qbar_levels = np.empty(J, dtype=float)
        for idx, fid in enumerate(firm_ids):
            if int(fid) not in id_to_idx:
                raise ValueError(f"firm_id {fid} not found in {firms_csv_path}")
            row = df_sorted.iloc[id_to_idx[int(fid)]]
            qbar_val = float(row[qbar_col])
            if qbar_val <= 0.0:
                qbar_val = float(np.nan)
            qbar_levels[idx] = qbar_val

        if np.any(~np.isfinite(qbar_levels)):
            raise ValueError("Encountered non-positive or missing cutoffs when reading firms CSV.")
        ln_qbar_guess = np.log(qbar_levels)

    theta_full = np.concatenate(([tau_hat], delta_hat, ln_qbar_guess))
    return theta_full
