...
parameters (tau, qbar, gamma1, sigma_e, gamma0).
...
try:
    ...
    from screening.analysis.lib.jax_model import choice_probabilities_from_cutoffs  # type: ignore
...
def solve_delta_contraction_single_market(
    ...
    def body_fn(carry):
        ...
        s_model = _model_shares(tau, delta, qbar, X_m, D_m, gamma1, sigma_e, gamma0)
...
def solve_delta_contraction_batched(
    ...
    for it in range(maxiter):
        delta, err = _contraction_step(
            ...
            log_s_data_batch, gamma1, sigma_e, gamma0,
        )
        if float(err) <= tol:
            break
