...
try:
    ...
    from .blp_contraction import solve_delta_contraction_batched
except ImportError:  # pragma: no cover - script execution fallback
    sys.path.append(str(Path(__file__).resolve().parents[2]))
    from code import get_data_subdir, get_output_subdir, DATA_RAW, DATA_CLEAN, DATA_BUILD, OUTPUT_ESTIMATION  # type: ignore
    from helpers import (
        compute_worker_firm_distances,  # type: ignore
        load_weight_matrix,  # type: ignore
        naive_theta_guess_tau_delta_qbar,  # type: ignore
    ...
    from blp_contraction import solve_delta_contraction_batched  # type: ignore
...
class Baseline:
    ...
    def vector(self) -> np.ndarray:
        head = np.array(
            [self.tau, self.alpha, self.gamma, self.sigma_e, self.lambda_e],
            dtype=float,
        )
...
class ModelData:
    ...
    mu_e_scalar: jnp.ndarray | None = None
...
def make_transform(J: int) -> ParameterTransform:
    ...
    def fwd(z: jnp.ndarray) -> jnp.ndarray:
        ...
        lambda_e = z[4]
        delta = z[5 : 5 + J]
        qbar = QBAR_FLOOR + jnn.softplus(z[5 + J :])
        return jnp.concatenate(
            (
                jnp.array(
                    [tau, alpha, gamma, sigma_e, lambda_e], dtype=jnp.float64
                ),
                delta,
                qbar,
            )
    ...
    def inv(theta: jnp.ndarray) -> jnp.ndarray:
        ...
        qbar_z = _softplus_inv(jnp.maximum(qbar - QBAR_FLOOR, 1e-12))

        return jnp.concatenate(
            (
                jnp.array(
                    [tau_z, alpha_z, gamma_z, sigma_e_z, lambda_e],
                    dtype=jnp.float64,
                ),
                delta,
                qbar_z,
...
def make_transform_no_delta(J: int) -> ParameterTransform:
    ...
    Parameter vector: [tau, alpha, gamma, sigma_e, lambda_e, qbar_1..J] (length 5+J).
    ...
    def fwd(z: jnp.ndarray) -> jnp.ndarray:
        ...
        sigma_e = POS_FLOOR + jnn.softplus(z[3])
        lambda_e = z[4]
        qbar = QBAR_FLOOR + jnn.softplus(z[5:])
        return jnp.concatenate(
            (
                jnp.array(
                    [tau, alpha, gamma, sigma_e, lambda_e], dtype=jnp.float64
...
def transform_theta_and_jacobian(theta: np.ndarray, J: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    tau, alpha, gamma, sigma_e, lambda_e = theta[:5]
    delta = theta[5 : 5 + J]
    qbar = theta[5 + J :]

    inv_sigma = 1.0 / sigma_e
    gamma_tilde = gamma * inv_sigma
    qbar_tilde = (qbar - lambda_e) * inv_sigma

    theta_tilde = theta.copy()
    theta_tilde[2] = gamma_tilde
    theta_tilde[5 + J :] = qbar_tilde

    jac = np.eye(K, dtype=float)
    jac[2, :] = 0.0
    jac[2, 2] = inv_sigma
    jac[2, 3] = -gamma * inv_sigma**2

    for j in range(J):
        row = 5 + J + j
        jac[row, :] = 0.0
        jac[row, 3] = -(qbar[j] - lambda_e) * inv_sigma**2
        jac[row, 4] = -inv_sigma
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Penalized JAX MLE for (tau, alpha, gamma, sigma_e, delta, qbar)",
    ...
    parser.add_argument("--theta0_list", type=str, default=None,
                        help="Comma-separated tau, alpha, gamma, sigma_e, delta(1..J), qbar(1..J).")
    parser.add_argument("--theta0_from_helper", action="store_true",
                        help="Initialise tau,delta,qbar via helpers.naive_theta_guess_tau_delta_qbar and append alpha,gamma,sigma_e baselines.")
    ...
    parser.add_argument("--freeze", type=str, default=None,
                        help="Comma-separated parameter names to freeze at initial values "
                             "(e.g. 'gamma,sigma_e,lambda_e'). Frozen params are held fixed during optimisation.")
...
def _enforce_feasible(theta: np.ndarray, J: int) -> np.ndarray:
    ...
    theta[5 + J :] = np.maximum(theta[5 + J :], QBAR_FLOOR + 1e-9)
    return theta
...
def _enforce_feasible_no_delta(theta: np.ndarray) -> np.ndarray:
    ...
    theta[2] = max(theta[2], POS_FLOOR + 1e-9)
    theta[3] = max(theta[3], POS_FLOOR + 1e-9)
    theta[5:] = np.maximum(theta[5:], QBAR_FLOOR + 1e-9)
    return theta
...
def prepare_problem(args: argparse.Namespace) -> Tuple[ModelData, Baseline, jnp.ndarray, jnp.ndarray, ParameterTransform]:
    ...
    gamma_baseline = params.get("gamma", 1.0)
    mu_e = params.get("mu_e", params.get("mu_a", 0.0))
    sigma_baseline = params.get("sigma_e", params.get("sigma_a", 0.12))
    tau_baseline = params.get("tau", params.get("gamma", 0.05))
    alpha_baseline = params.get("alpha", params.get("beta", 0.5))

    alpha_init = float(alpha_baseline)
    gamma_init = float(gamma_baseline)
    sigma_e_init = float(sigma_baseline)
    lambda_e_init = 0.0

    eta_param = params.get("eta", 1.0)
    delta0_nat = eta_param * np.log(np.maximum(w, 1e-300)) + xi
    qbar0_nat = np.maximum(qbar_data, 1e-10)
    ...
    theta0_np = _initial_theta(
        ...
        alpha_init, gamma_init, sigma_e_init, lambda_e_init, helper_kwargs,
    )
    ...
    if is_multi_market:
        ...
        model_data = ModelData(
            ...
            mu_e_scalar=jnp.asarray(float(mu_e), dtype=jnp.float64),
        ...
        model_data = ModelData(
            ...
            aux_template={
                ...
                "mu_e": jnp.asarray(float(mu_e), dtype=jnp.float64),
    ...
    if delta_method == "contraction":
        # Contraction method: theta = [tau, alpha, gamma, sigma_e, lambda_e, qbar_1..J]
        theta0_no_delta = np.concatenate((
            theta0_np[:5],     # tau, alpha, gamma, sigma_e, lambda_e
            theta0_np[5 + J:], # qbar
        ))
...
def make_components_fn(data: ModelData):
    ...
    def components(theta: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]:
        ...
        adjustment = Q_nat - lambda_e - alpha_term * qbar_nat + lambda_e * alpha_term
...
def make_components_fn_batched_contraction(data: ModelData, contraction_tol: float = 1e-12,
    ...
    The theta vector has shape (5+J,): [tau, alpha, gamma, sigma_e, lambda_e, qbar_1..J].
    ...
    """
    from code.estimation.jax_model import compute_penalty_components_jax
    from code.estimation.blp_contraction import _contraction_step, _model_shares_vmapped
    ...
    def components(theta: jnp.ndarray):
        ...
        for it in range(contraction_maxiter):
            delta, err = _contraction_step(
                ...
                log_s_data_batch, gamma, sigma_e_fixed, data.mu_e_scalar,
            )
...
def build_frozen_mask(freeze_str: str | None, J: int) -> tuple[jnp.ndarray, list[str]]:
    ...
    scalar_map = {"tau": 0, "alpha": 1, "gamma": 2, "sigma_e": 3, "lambda_e": 4}
    frozen_names = []
...
def make_contraction_value_and_grad_z(
    ...
    """
    from code.estimation.jax_model import compute_penalty_components_jax
    from code.estimation.blp_contraction import _contraction_step
    ...
    def _objective_given_delta(theta: jnp.ndarray, delta_batch: jnp.ndarray):
        ...
        nlls, Q_batch, L_batch = _vmapped_core(
            ...
            data.mu_e_scalar, gamma, sigma_e_fixed,
        )
    ...
    def value_and_grad_z(z: jnp.ndarray):
        ...
        for it in range(contraction_maxiter):
            delta, err = _contraction_step(
                delta, tau, qbar_batch, data.X_batch, data.D_batch,
                log_s_data, gamma, sigma_e_fixed, data.mu_e_scalar,
            )
            err_val = float(err)
...
def distance_metrics(theta_hat: np.ndarray, baseline: Baseline) -> Dict[str, float]:
    ...
    return {
        ...
        "gamma_hat": float(theta_hat[2]),
        "gamma_true": float(baseline.gamma),
        "gamma_error": float(theta_hat[2] - baseline.gamma),
...
def make_param_names_transformed(J: int) -> list[str]:
    return (
        ["tau", "alpha", "gamma_over_sigma_e", "sigma_e", "lambda_e"]
        + [f"delta_{idx + 1}" for idx in range(J)]
...
def main() -> None:
    ...
    if use_contraction:
        ...
        delta_final, conv_final = solve_delta_contraction_batched(
            ...
            jnp.asarray(gamma_hat, dtype=jnp.float64),
            sigma_e_fixed,
            data.mu_e_scalar,
        ...
        # Build full theta: [tau, alpha, gamma, sigma_e, lambda_e, delta_1..J, qbar_1..J]
    ...
    if args.skip_plot:
        ...
        plot_scalar(axes[2], 2, "gamma/sigma_e", theta_hat_tilde_np[2], baseline_tilde_np[2])
    ...
    out = {
        ...
        "true_params_transformed": {
            ...
            "gamma_over_sigma_e": baseline_tilde_np[2],
