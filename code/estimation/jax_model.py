...
- theta: 1D array with length 1 + 2J in natural firm order
         theta = [tau, delta_nat(1..J), qbar_nat(1..J)]
...
def _choice_probabilities_from_cutoffs(
    ...
    gamma = jnp.asarray(aux.get("gamma", 1.0), dtype=jnp.float64)
    mu_e = jnp.asarray(aux.get("mu_e", 0.0), dtype=jnp.float64)
    ...
    qbar_nat = jnp.asarray(qbar_nat).reshape((J,))

    maps = _order_maps_jax(qbar_nat)
    order_idx = maps["order_idx"]
    inv_order = maps["inv_order"]
    qbar_sorted = maps["qbar_sorted"]
    ...
    big = jnp.array(1e6, dtype=qbar_sorted.dtype)
    qbar_pad = jnp.concatenate(
        (jnp.array([-big], dtype=qbar_sorted.dtype), qbar_sorted, jnp.array([big], dtype=qbar_sorted.dtype))
    )

    x_skill = jnp.asarray(X).reshape((N,))
    s = gamma * x_skill + mu_e

    z_hi = (qbar_pad[1:][None, :] - s[:, None]) / sigma_e
    z_lo = (qbar_pad[:-1][None, :] - s[:, None]) / sigma_e
...
def compute_choice_probabilities_jax(theta: jnp.ndarray, X: jnp.ndarray, aux: Dict) -> jnp.ndarray:
    ...
    Args:
      ...
      aux:   dict with at least {D_nat, gamma, mu_e, sigma_e}
...
def compute_choice_probabilities_tau_qbar_delta_jax(theta: jnp.ndarray, X: jnp.ndarray, aux: Dict) -> jnp.ndarray:
    ...
    Args:
      ...
      aux:   dict with at least {D_nat, gamma, mu_e, sigma_e}; other keys ignored
...
def compute_penalty_components_jax(
    ...
    big = jnp.array(1e6, dtype=qbar_sorted.dtype)
    qbar_pad = jnp.concatenate(((-big)[None], qbar_sorted, big[None]))

    gamma_x = gamma * X
    z_hi = (qbar_pad[1:] - gamma_x[:, None] - mu_e) / sigma_e
    z_lo = (qbar_pad[:-1] - gamma_x[:, None] - mu_e) / sigma_e
    ...
    ability_weight_term = mu_e * p_x + sigma_e * (pdf_lo - pdf_hi)
    ...
    skill_contrib_sorted = gamma_x[:, None] * joint_prob_sum + ability_weighted_sorted
...
def compute_penalty_components_with_params_jax(
    ...
    aux_dynamic["gamma"] = jnp.asarray(gamma, dtype=jnp.float64)
