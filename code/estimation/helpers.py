...
def compute_p_x_probabilities(x_skill: float, qbar_sorted: np.ndarray, gamma: float, mu_e: float, sigma_e: float) -> np.ndarray:
    """
    Compute skill-based probabilities p_x using truncated normal CDF differences.

    Args:
        x_skill: Worker skill level
        qbar_sorted: Sorted screening thresholds (J,)
        gamma: Skill loading parameter
        mu_e: Mean of preference shock
        sigma_e: Standard deviation of preference shock
    ...
    # Compute a(x) = gamma*x + mu_e
    a_x = gamma * x_skill + mu_e
...
def compute_choice_probabilities(tau: float, delta: np.ndarray, distances: np.ndarray,
                               ...
                               alpha: float, gamma: float, mu_e: float, sigma_e: float,
    ...
    Args:
        ...
        Y: Firm output (J,)
        A: Firm TFP (J,)
        qbar: Firm screening thresholds (J,)
        x_skill: Worker skills (N,)
        firm_ids: Firm IDs (J,)
        alpha: DRS curvature parameter
        gamma: Skill loading parameter
        mu_e: Mean of preference shock
        sigma_e: Std dev of preference shock
...
def create_probability_evaluator(delta: np.ndarray, distances: np.ndarray, w: np.ndarray,
                               ...
                               firm_ids: np.ndarray, alpha: float, gamma: float, mu_e: float, sigma_e: float) -> Callable:
    """
    ...
    def probs_worker_level(theta) -> np.ndarray:
        ...
        return compute_choice_probabilities(tau, delta, distances, w, Y, A, qbar, x_skill,
                                          firm_ids, alpha, gamma, mu_e, sigma_e, delta_nat)
...
def naive_theta_guess_tau_delta_qbar(
    ...
    *,
    ...
    """Naive theta_0 = (tau, delta, qbar) using choices, locations, and observables.

    If *gamma* and *alpha* are provided, qbar is computed from the screening FOC
    using only observed data (Y, w, worker matches):

        H_j  = sum_{i matched to j} gamma * x_skill_i
        ln A_j = ln Y_j - (1 - alpha) * ln H_j
        ln qbar_j = ln w_j - ln A_j - ln(1-alpha) + alpha * ln H_j

    Otherwise falls back to reading qbar from the firms CSV.
    """
    ...
    if gamma is not None and alpha is not None:
        # --- Compute qbar from screening FOC using observables only ---
        # H_j = sum_{i matched to j} gamma * x_skill_i
        H = np.zeros(J, dtype=float)
        for idx, fid in enumerate(firm_ids):
            mask = chosen_firm == int(fid)
            if np.any(mask):
                H[idx] = gamma * np.sum(x_skill[mask])
        ...
        # ln qbar_j = ln w_j - ln A_j - ln(1-alpha) + alpha * ln H_j
        ln_qbar = (np.log(np.maximum(w, 1e-300))
                    - ln_A
                    - np.log(1.0 - alpha)
                    + alpha * ln_H)
        qbar_guess = np.exp(ln_qbar)
        qbar_guess = np.maximum(qbar_guess, 1e-8)

        if verbose:
            print(f"[naive_qbar] H range: [{H.min():.2f}, {H.max():.2f}]")
            print(f"[naive_qbar] qbar range: [{qbar_guess.min():.4f}, {qbar_guess.max():.4f}]")
    else:
        # --- Fall back to reading qbar from firms CSV ---
        ...
        qbar_guess = np.empty(J, dtype=float)
        for idx, fid in enumerate(firm_ids):
            ...
            qbar_guess[idx] = qbar_val

        if np.any(~np.isfinite(qbar_guess)):
            raise ValueError("Encountered non-positive or missing cutoffs when reading firms CSV.")

    theta_full = np.concatenate(([tau_hat], delta_hat, qbar_guess))
    return theta_full
